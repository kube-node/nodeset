...
// nodeClassLister implements the NodeClassLister interface.
type nodeClassLister struct {
	indexer cache.Indexer
}

// NewNodeClassLister returns a new NodeClassLister.
func NewNodeClassLister(indexer cache.Indexer) NodeClassLister {
	return &nodeClassLister{indexer: indexer}
}

// List lists all NodeClasses in the indexer.
func (s *nodeClassLister) List(selector labels.Selector) (ret []*v1alpha1.NodeClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.NodeClass))
	})
	return ret, err
}

// Get retrieves the NodeClass from the index for a given name.
func (s *nodeClassLister) Get(name string) (*v1alpha1.NodeClass, error) {
	key := &v1alpha1.NodeClass{ObjectMeta: v1.ObjectMeta{Name: name}}
	obj, exists, err := s.indexer.Get(key)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("nodeclass"), name)
	}
	return obj.(*v1alpha1.NodeClass), nil
}
